...
class HelpDialog(gui.Dialog):
	def __init__(self):
		...
		c = gui.Container(background = (0,0,0))
		
		### Main text
		txt = "".join(["Eit is a tetris version based on Eittris ", 
		...
		### Special Blocks
		t = gui.Table(width = 250, height = 320, background = (0,0,0))
		t.tr()
		...
		t.td(gui.Label( "Colors", color = txtcolor), align = -1)
...
class ManageProfilesDialog(gui.Dialog):
	def __init__(self):
		...
		width = 500
		height = 300
		t = gui.Table(width=width, height = height)
		...
		### New Profile
		doc = gui.Document(width = 100)
		t2 = gui.Table(width = 100, height = 100)
		
		t2.tr()
		t2.td(gui.Label("Name: "), align=1)
		...
		t2.tr()
		t2.td(gui.Label("Left: "), align=1)
		self.left = gui.Keysym()
		t2.td(self.left)
		
		t2.tr()
		t2.td(gui.Label("Right: "), align=1)
		self.right = gui.Keysym()
		t2.td(self.right)
		
		t2.tr()
		t2.td(gui.Label("Rotate CW: "), align=1)
		self.cw = gui.Keysym()
		t2.td(self.cw)
		
		t2.tr()
		t2.td(gui.Label("Rotate CCW: "), align=1)
		self.ccw = gui.Keysym()
		t2.td(self.ccw)
		
		t2.tr()
		t2.td(gui.Label("Block Down: "), align=1)
		self.down = gui.Keysym()
		t2.td(self.down)
		
		t2.tr()
		t2.td(gui.Label("Block Drop: "), align=1)
		self.drop = gui.Keysym()
		t2.td(self.drop)
		
		t2.tr()
		t2.td(gui.Label("Use Antidote: "), align=1)
		self.use_anti = gui.Keysym()
		t2.td(self.use_anti)
		
		t2.tr()
		t2.td(gui.Label("Change Target: "), align=1)
		...
		doc.add( gui.Label("Available Profiles") )
...
class SelectProfileDialog(gui.Dialog):
	def __init__(self):
		...
		profiles = ConfigObj("profiles.cfg")
		self.profile_list = gui.List(width=145, height=100)
		names = profiles.keys()
		names.sort()
		for name in names:
			self.profile_list.add(name, value = name)
		...
		b = gui.Button("Cancel", width=50)
		b.connect(gui.CLICK,self.close,None)
		doc.add(b)
		doc.space(space)
		ok_button = gui.Button("Ok", width=50)
...
class ViewScoreDialog(gui.Dialog):
	def __init__(self,**params):
		...
		t = gui.Table(width=300, height=150)
		t.tr()
		t.td( gui.Label("Place") )
		t.td( gui.Label("Score") )
		t.td( gui.Label("Name") )
		t.tr()
		...
		newhs = []
		for h in hs:
			newhs.append((h.score, h.name))
		newhs.sort()
		newhs.reverse()
		for s,n in newhs:
			t.tr()
			t.td(gui.Label(str(place)))
			t.td(gui.Label(str(s)))
			t.td(gui.Label(str(n)))			
			place +=1
		doc.add(t)
		gui.Dialog.__init__(self, title, gui.ScrollArea(doc,width,height))
